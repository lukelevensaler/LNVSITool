...
class AnalysisEngine:
	...
	def __init__(self, ui):
		...
		self.ppr = Preprocessor(ui, self.rh)
	...
	def read_csv(self):
		try:
			...
			self.ppr.wavelengths = np.asarray(wavelengths)  # Pass wavelengths to preprocessor as numpy array
			...
			if self.positive_control not in self.processed:
				try:
					...
					x_smooth, y_smooth, sg_win, sg_poly = self.ppr.smooth_curves(x, y)
	...
	def preprocess_all_curves(self, data):
		try:
			...
			optimal_win, optimal_poly = self.ppr.bayesian_savgol_optimize_win_and_poly(data, self.wavelengths)
			...
			for name, y in data.items():
				try:
					...
					x_smooth, y_smooth, sg_win, sg_poly = self.ppr.smooth_curves(x, y, win_opt=optimal_win, poly_opt=optimal_poly)
	...
	def analyze_all_samples(self):
		try:
			...
			for c in centers:
				fitted_seg = self.ppr.deconvolve_voigt(x_ctrl, y_ctrl_raw, center_nm=c, window=window, baseline=y_ctrl_sg, sg_win=sg_win_ctrl, sg_poly=sg_poly_ctrl)
			...
			try:
				...
				aligned_control = self.ppr.align_curves_dtw(y_ctrl_full, y_ctrl_full)
			...
			for idx, name in enumerate(self.sample_names):
				...
				for c in centers:
					fitted_seg = self.ppr.deconvolve_voigt(x_sample, y_sample_raw, center_nm=c, window=window, baseline=y_sample_sg, sg_win=sg_win_sample, sg_poly=sg_poly_sample)
					mask = ~np.isnan(fitted_seg)
					if np.any(mask):
						fitted_sample[mask] = fitted_seg[mask]
				# DTW-align reconstructed sample to control and store for saving
				try:
					aligned_sample = self.ppr.align_curves_dtw(y_ctrl_full, fitted_sample)
				except Exception:
